...
const ACTION_LABELS: Record<IgCommentActionType, string> = {
  PRIVATE_REPLY: "Private DM",
  PUBLIC_COMMENT: "Public Reply",
  BOTH: "DM + Reply",
};

const ACTION_COLORS: Record<IgCommentActionType, string> = {
...
function LogsDialog({
  ...
  automation: IgCommentAutomation;
  onClose: () => void;
}) {
  const { data: events, isLoading } = useQuery({
    queryKey: qk.igCommentEvents(automation.id),
    queryFn: async () => {
      const { data } = await api.get<IgCommentEvent[]>(
        `/ig-comment-automations/${automation.id}/events?take=100`,
      );
      return data;
    },
  });
...
function AutomationDialog({
  ...
  const [form, setForm] = useState<AutomationForm>(() => {
    ...
    return {
      ...EMPTY_FORM,
      instagramAccountId: preset?.instagramAccountId ?? "",
      postId: preset?.postId ?? "",
      postSnippet: preset?.postSnippet ?? "",
    };
  });

  const accountIsLocked = isEditing || !!preset?.instagramAccountId;
  ...
  const lockedAccountName =
    accounts.find((a) => a.id === form.instagramAccountId)?.username
      ? `@${accounts.find((a) => a.id === form.instagramAccountId)!.username}`
      : accounts.find((a) => a.id === form.instagramAccountId)?.name ??
        initial?.igAccount?.username
          ? `@${initial?.igAccount?.username}`
          : initial?.igAccount?.name ??
  ...
  const {
    ...
    queryKey: qk.igAccountPosts(form.instagramAccountId ?? ""),
    queryFn: async () => {
      if (!form.instagramAccountId) return [];
      const { data } = await api.get<IgPost[]>(
        `/ig-comment-automations/accounts/${form.instagramAccountId}/posts`,
      );
      return data;
    },
    enabled: !!form.instagramAccountId && !postIsLocked,
  });
  ...
  const saveMutation = useMutation({
    mutationFn: async (payload: AutomationForm) => {
      ...
      if (isEditing) {
        await api.patch(`/ig-comment-automations/${initial!.id}`, body);
      } else {
        await api.post("/ig-comment-automations", body);
      }
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: qk.igCommentAutomations });
  ...
  const handleSelectPost = (post: IgPost) => {
    setForm((f) => ({
      ...f,
      postId: post.postId,
  ...
  return (
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-base font-bold">
            {isEditing ? "Edit Automation" : "New IG Comment Automation"}
          </DialogTitle>
        </DialogHeader>
            ...
            <div className="space-y-1.5">
              ...
              {accountIsLocked ? (
                ...
                <select
                  ...
                  onChange={(e) => {
                    setField("instagramAccountId", e.target.value);
                    setField("postId", "");
              ...
              {!accountIsLocked && !accounts.length && (
            ...
            <div className="space-y-1.5">
              ...
              {loadingPosts ? (
                ...
                <p className="text-xs text-[#9CA3AF] py-2">
                  No posts found. Leave blank to match all posts.
                </p>
              ) : (
              ...
              <p className="text-[10px] text-[#9CA3AF]">
                Leave blank to trigger on comments from any post.
              </p>
            </div>
          ) : null}
            ...
            <div className="space-y-4 rounded-xl border border-[#E5E7EB] dark:border-[#1e2433] p-3.5">
              ...
              <div className="space-y-1.5">
                ...
                <Textarea
                  ...
                  placeholder="Hi! Thanks for your interest. Check your DMs for details 👇"
                  rows={2}
                  className="text-sm"
                />
                <p className="text-[10px] text-[#9CA3AF]">
                  Sent as a private Instagram DM to the commenter.
                </p>
              </div>
              <div className="space-y-1.5">
              ...
              <Textarea
                ...
                placeholder={
                  form.actionType === "PUBLIC_COMMENT"
                    ...
                    : "Hi! Thanks for your interest. We'll DM you the details."
                }
                rows={3}
                className="text-sm"
              />
...
function PostFeedPanel({
  ...
  onAddAutomation: (post: IgPost) => void;
}) {
  const {
    data: posts = [],
    isFetching,
    ...
    queryKey: qk.igAccountPosts(accountDbId ?? ""),
    queryFn: async () => {
      if (!accountDbId) return [];
      const { data } = await api.get<IgPost[]>(
        `/ig-comment-automations/accounts/${accountDbId}/posts`,
      );
      return data;
    },
    enabled: !!accountDbId,
      ...
      <div className="flex-1 overflow-y-auto">
        {!accountDbId ? (
          <div className="flex flex-col items-center justify-center h-48 px-4 text-center text-[#9CA3AF]">
            <Camera className="size-6 mb-2 opacity-30" />
            <p className="text-xs">Select an account to see posts</p>
          </div>
            ...
            {posts.map((post) => (
              ...
              >
                <div className="size-10 rounded-lg bg-[#F3F4F6] dark:bg-[#1e2433] shrink-0 mt-0.5 flex items-center justify-center">
                  <Camera className="size-4 text-[#9CA3AF]" />
                </div>
                <div className="flex-1 min-w-0">
...
export default function IgCommentAutomationsPage() {
  const queryClient = useQueryClient();
  ...
  const [editTarget, setEditTarget] = useState<IgCommentAutomation | null>(null);
  const [logsTarget, setLogsTarget] = useState<IgCommentAutomation | null>(null);
  ...
  const { data: automations = [], isLoading: loadingAutomations } = useQuery({
    queryKey: qk.igCommentAutomations,
    queryFn: async () => {
      const { data } =
        await api.get<IgCommentAutomation[]>("/ig-comment-automations");
      return data;
    },
  });
  ...
  const toggleMutation = useMutation({
    mutationFn: (id: string) =>
      api.post(`/ig-comment-automations/${id}/toggle`),
    onSuccess: () =>
      queryClient.invalidateQueries({ queryKey: qk.igCommentAutomations }),
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => api.delete(`/ig-comment-automations/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: qk.igCommentAutomations });
  ...
  const accountAutomations = automations.filter(
    (a) => a.instagramAccountId === selectedAccountId,
  );
  const activeCount = accountAutomations.filter((a) => a.isActive).length;
  const totalReplies = accountAutomations.reduce(
    (sum, a) => sum + a.replyCount,
    0,
  );

  const handleDelete = (automation: IgCommentAutomation) => {
  ...
  const handleAddFromPost = (post: IgPost) => {
  ...
  const getAccountDisplayName = (account: InstagramAccount) =>
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-[#111827] dark:text-[#f3f4f6]">
            IG Comment Automations
          </h2>
          <p className="text-sm text-[#6B7280] mt-0.5">
            Auto-reply when someone comments a keyword on your Instagram posts
          </p>
        </div>
        <Button
          onClick={() => openCreate()}
          ...
          <div className="flex-1 overflow-y-auto">
            {loadingAccounts ? (
              ...
              <div className="px-4 py-6 text-center">
                <Camera className="size-6 text-[#9CA3AF] mx-auto mb-2 opacity-40" />
              ...
              <div className="py-2">
                {accounts.map((account) => {
                  const count = automations.filter(
                    (a) => a.instagramAccountId === account.id,
                  ).length;
        ...
        <div className="flex-1 min-w-0 flex flex-col overflow-hidden">
          {!selectedAccount ? (
            <div className="flex flex-col items-center justify-center flex-1 py-20 text-center text-[#9CA3AF]">
              ...
              <p className="text-sm">Select an account from the sidebar</p>
            </div>
          ) : (
            <>
              ...
              <div className="flex items-center justify-between px-5 py-3 border-b border-[#E5E7EB] dark:border-[#1e2433] shrink-0 bg-white dark:bg-[#111420]">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] bg-[#F3F4F6] dark:bg-[#1e2433] text-[#6B7280] px-2 py-0.5 rounded-full font-semibold">
                    {accountAutomations.length} total
                  </span>
                </div>
                <Button
                  size="sm"
                  onClick={() =>
                    openCreate({ instagramAccountId: selectedAccountId! })
                  }
