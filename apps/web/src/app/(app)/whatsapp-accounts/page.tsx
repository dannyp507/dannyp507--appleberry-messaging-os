...
export default function WhatsAppAccountsPage() {
  ...
  // No origin check — the message may come from web.facebook.com, staticxx.facebook.com, etc.
  useEffect(() => {
  ...
  const embeddedSignupMutation = useMutation({
    ...
    onError: (e: unknown) => toast.error(getApiErrorMessage(e) || "Meta signup failed — please try manual credentials"),
  });
  ...
  const launchEmbeddedSignup = () => {
    ...
    window.FB.login((response: any) => {
      // The OAuth code to exchange server-side for a real access token
      const code: string | undefined = response?.authResponse?.code ?? undefined;

      // Poll for WA_EMBEDDED_SIGNUP postMessage (phone_number_id + waba_id).
      // The message arrives async after the popup closes, so we retry every 500 ms
      // for up to 3 seconds.
      let attempts = 0;
      const check = () => {
        ...
        if (embedded?.phoneNumberId) {
          // ✅ Best case — have everything. Fully automatic, no user input needed.
          embeddedSignupMutation.mutate({
            code,
            phoneNumberId: embedded.phoneNumberId,
            wabaId: embedded.wabaId || undefined,
          });
        } else if (attempts < 6) {
          attempts++;
          setTimeout(check, 500);
        } else {
          // WA_EMBEDDED_SIGNUP never arrived — fall back to manual form.
          // If we at least have a code, auto-fill a note for the user.
          if (code) {
            toast.info("Meta login successful — enter your Phone Number ID below to finish.");
          } else {
            toast.error("Could not complete Meta signup — please fill the fields manually.");
          }
        }
      };
      setTimeout(check, 500);
                      ...
                      {META_APP_ID && META_CONFIG_ID && (
                        <>
                          <button
                            ...
                            className={cn(
                              "relative flex w-full items-center gap-0 overflow-hidden rounded-xl border-0 bg-[#1877F2] text-sm font-semibold text-white transition-all hover:bg-[#166FE5] active:bg-[#1464D8] disabled:opacity-60 shadow-md hover:shadow-lg",
                            )}
                          >
                            {/* Facebook "f" logo box */}
                            <span className="flex size-12 shrink-0 items-center justify-center bg-white/15">
                              {embeddedSignupMutation.isPending ? (
                                ...
                                <svg viewBox="0 0 24 24" className="size-5 fill-white" aria-hidden="true">
                                  <path d="M24 12.073C24 5.405 18.627 0 12 0S0 5.405 0 12.073C0 18.1 4.388 23.094 10.125 24v-8.437H7.078v-3.49h3.047V9.41c0-3.025 1.791-4.697 4.533-4.697 1.312 0 2.686.235 2.686.235v2.97h-1.513c-1.491 0-1.956.93-1.956 1.886v2.267h3.328l-.532 3.49h-2.796V24C19.612 23.094 24 18.1 24 12.073z"/>
                                </svg>
                              )}
                            </span>
                            <span className="flex-1 py-3.5 text-center tracking-wide">
                              {embeddedSignupMutation.isPending
                                ? "Connecting via Meta…"
                                : "Continue with Meta"}
                            </span>
                            {!fbSdkReady && !embeddedSignupMutation.isPending && (
            ...
            return (
              ...
              >
                ...
                <div className="flex gap-2 border-t border-[#E5E7EB] px-4 py-3">
                  ...
                  <Link href={`/whatsapp-accounts/${a.id}`} title="AI Settings">
                    <Button
                      size="sm"
                      variant="outline"
                      className="rounded-xl border-[#E5E7EB] bg-[#F9FAFB] text-[#6B7280] hover:border-violet-400/40 hover:bg-violet-500/5 hover:text-violet-500 px-2"
                    >
