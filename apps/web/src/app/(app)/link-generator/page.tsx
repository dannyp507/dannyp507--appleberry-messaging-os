...
export default function LinkGeneratorPage() {
  ...
  return (
    <div className="page-container space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-white">Link & QR Generator</h2>
        <p className="text-sm text-[#a9abb3] mt-1">Generate click-to-chat links and QR codes for your WhatsApp numbers</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Config panel */}
        <div className="bg-[#161a21] border border-[#262B33]/30 rounded-2xl p-6 space-y-5">
          <h3 className="text-[10px] font-bold text-[#73757d] uppercase tracking-widest">Configuration</h3>

          <div className="space-y-1.5">
            <label className="text-xs font-bold text-[#73757d] uppercase tracking-widest">WhatsApp Account</label>
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-[#73757d] uppercase tracking-widest">Pre-filled Message <span className="normal-case font-normal">(optional)</span></label>
          ...
          {waLink && (
            <div className="space-y-2">
              <label className="text-xs font-bold text-[#73757d] uppercase tracking-widest">Generated Link</label>
              <div className="flex gap-2">
                <div className="flex-1 bg-[#0B0E14] border border-[#262B33]/40 rounded-lg px-3 py-2.5 text-xs text-[#a9abb3] font-mono break-all">
                  {waLink}
                </div>
                <button onClick={copy}
        ...
        {/* QR preview */}
        <div className="bg-[#161a21] border border-[#262B33]/30 rounded-2xl p-6 flex flex-col items-center justify-center gap-6">
          <h3 className="text-[10px] font-bold text-[#73757d] uppercase tracking-widest self-start">QR Code Preview</h3>
          {waLink ? (
            <>
              <div className="p-4 bg-[#161a21] border border-[#262B33]/40 rounded-xl">
              ...
              {selectedAccount && (
                <p className="text-xs text-[#73757d] text-center">
                  Scan to open WhatsApp and start a chat with <span className="text-white font-medium">{selectedAccount.name}</span>
                </p>
              )}
            </>
          ) : (
            <div className="flex flex-col items-center justify-center text-center py-10">
              <div className="w-36 h-36 rounded-xl border-2 border-dashed border-[#262B33] flex items-center justify-center mb-4">
                <span className="material-symbols-outlined text-4xl text-[#262B33]">qr_code_2</span>
              </div>
              <p className="text-sm text-[#73757d]">Select an account to generate a QR code</p>
            </div>
          )}
        </div>
      </div>
