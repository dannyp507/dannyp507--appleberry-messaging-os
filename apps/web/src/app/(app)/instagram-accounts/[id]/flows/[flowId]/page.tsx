...
import type { ChatbotFlowDetail, InstagramAccount } from "@/lib/api/types";
...
export default function IgFlowBuilderPage() {
  ...
  if (error || !flow) {
    return (
      <div className="h-full flex flex-col items-center justify-center gap-3 text-center p-8">
        ...
        <Link
          href={`/instagram-accounts/${id}/flows`}
    ...
    <div className="flex flex-col" style={{ height: "calc(100vh - 56px)" }}>
      ...
      <header className="shrink-0 h-12 flex items-center gap-3 px-4 bg-white border-b border-zinc-200 z-10">
        ...
        >
          ...
          <span className="hidden sm:inline">{account?.name ?? "Flows"}</span>
        </Link>
