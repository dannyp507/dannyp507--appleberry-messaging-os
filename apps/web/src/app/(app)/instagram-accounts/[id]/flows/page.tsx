...
import type { ChatbotFlowSummary, InstagramAccount } from "@/lib/api/types";
...
export default function IgAccountFlowsPage() {
  ...
  const { data: flows = [], isLoading } = useQuery({
    queryKey: qk.igAccountFlows(id),
    queryFn: async () => {
      const { data } = await api.get<ChatbotFlowSummary[]>(
        `/chatbot/flows?instagramAccountId=${id}`,
      );
      return data;
    },
    enabled: !!id,
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (flow) => {
      queryClient.invalidateQueries({ queryKey: qk.igAccountFlows(id) });
      setShowCreate(false);
      setNewFlowName("");
      router.push(`/instagram-accounts/${id}/flows/${flow.id}`);
    },
    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold">Instagram Flows</h1>
            <p className="text-sm text-muted-foreground">
              Visual chatbot flows for {account?.name ?? "this account"}
            </p>
          </div>
        </div>
        <button
          onClick={() => setShowCreate(true)}
          className="flex items-center gap-2 bg-pink-600 hover:bg-pink-700 text-white rounded-xl px-4 py-2.5 text-sm font-semibold transition-all shadow-lg shadow-pink-500/20"
        >
        ...
        <div className="mb-6 p-4 rounded-2xl border border-border/50 bg-card/50 backdrop-blur-sm">
          ...
          <div className="flex gap-3">
            <input
              ...
              className="flex-1 rounded-xl border border-border/50 bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-pink-500/50"
            />
            <button
              onClick={() => {
              ...
              className="px-4 py-2 bg-pink-600 hover:bg-pink-700 text-white rounded-xl text-sm font-semibold disabled:opacity-50 transition-all flex items-center gap-2"
            >
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-sm text-zinc-500 mt-1 max-w-xs">
            Create your first Instagram chatbot flow. Drag and drop blocks to build
            automated conversations.
          </p>
          <button
            onClick={() => setShowCreate(true)}
            className="mt-4 flex items-center gap-2 bg-pink-600 hover:bg-pink-700 text-white rounded-xl px-4 py-2.5 text-sm font-semibold transition-all"
          >
            <Plus className="size-4" />
            Create First Flow
          </button>
        ...
        <div className="grid gap-3">
          {flows.map((flow) => (
            <div
              ...
              onClick={() => router.push(`/instagram-accounts/${id}/flows/${flow.id}`)}
            >
              {/* Status dot */}
              <div
                className={cn(
