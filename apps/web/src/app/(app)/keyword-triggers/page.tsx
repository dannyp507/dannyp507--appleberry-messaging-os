  ...
  return (
    <div className="page-container space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-white">Keyword Triggers</h2>
          <p className="text-sm text-[#a9abb3] mt-1">Send automatic replies when a keyword is received</p>
        </div>
      ...
      {isLoading ? (
        <div className="space-y-3 animate-pulse">
          {[...Array(3)].map((_, i) => <div key={i} className="h-16 bg-[#161a21] rounded-xl" />)}
        </div>
      ) : triggers.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <span className="material-symbols-outlined text-5xl text-[#262B33] mb-4">tag</span>
          <p className="text-[#a9abb3]">No keyword triggers yet.</p>
          <p className="text-xs text-[#73757d] mt-1">When a contact sends a keyword, they automatically receive your reply.</p>
        </div>
      ) : (
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-[#161a21] border border-[#262B33]/40 rounded-2xl w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between p-6 border-b border-[#262B33]/20">
              <h3 className="text-lg font-bold text-white">New Keyword Trigger</h3>
            ...
            <div className="p-6 space-y-4">
              <div className="space-y-1.5">
                ...
                <p className="text-[10px] text-[#73757d]">Case-insensitive. Matches exact word in message.</p>
              </div>
              <div className="space-y-1.5">
