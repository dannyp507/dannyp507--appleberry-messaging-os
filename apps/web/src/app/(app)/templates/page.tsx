    ...
    <div className="page-container space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-[#a9abb3] mt-1">Message templates used in campaigns</p>
        </div>
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 animate-pulse">
          {[...Array(6)].map((_, i) => <div key={i} className="h-40 bg-[#161a21] rounded-xl" />)}
        </div>
      ) : templates.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <span className="material-symbols-outlined text-5xl text-[#262B33] mb-4">layers</span>
          <p className="text-[#a9abb3]">No templates yet. Create your first one.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {templates.map((t) => (
            <div key={t.id} className="bg-[#161a21] border border-[#262B33]/30 rounded-xl p-5 flex flex-col gap-3">
              <div className="flex items-start justify-between gap-2">
                <p className="font-semibold text-white text-sm leading-snug">{t.name}</p>
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider shrink-0 ${TYPE_COLORS[t.type as TemplateType] ?? "bg-[#262B33] text-[#a9abb3]"}`}>
                  {TYPE_LABELS[t.type as TemplateType] ?? t.type}
                </span>
              </div>
              {t.header && <p className="text-xs font-semibold text-[#a9abb3] truncate">{t.header}</p>}
              <p className="text-xs text-[#73757d] line-clamp-3">{t.content}</p>
              {t.footer && <p className="text-[10px] text-[#73757d]/70 italic">{t.footer}</p>}
              ...
              {t.sections && t.sections.length > 0 && (
                <div className="text-[10px] text-[#73757d]">
                  {t.sections.length} section{t.sections.length > 1 ? "s" : ""} · {t.sections.reduce((a, s) => a + s.rows.length, 0)} items
                </div>
              )}
              <div className="flex gap-2 mt-auto pt-2 border-t border-[#262B33]/20">
                <button onClick={() => setPreviewId(t.id)}
                  className="flex-1 py-1.5 rounded-lg bg-[#1c2028] text-xs text-[#a9abb3] hover:text-white transition-colors">Preview</button>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-[#161a21] border border-[#262B33]/40 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="flex items-center justify-between p-6 border-b border-[#262B33]/20 sticky top-0 bg-[#161a21] z-10">
              <h3 className="text-lg font-bold text-white">Create Template</h3>
              <button onClick={() => { setOpen(false); resetForm(); }} className="text-[#a9abb3] hover:text-white">
                <span className="material-symbols-outlined">close</span>
              </button>
            </div>
            <div className="p-6 space-y-5">
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-[#73757d] uppercase tracking-widest">Name</label>
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-[#73757d] uppercase tracking-widest">Type</label>
                <div className="grid grid-cols-4 gap-2">
                  {(["TEXT", "MEDIA", "BUTTON", "LIST"] as TemplateType[]).map((t) => (
                    <button key={t} onClick={() => setType(t)}
                      className={`py-2 rounded-lg text-xs font-bold transition-all ${type === t ? "stitch-gradient text-white" : "bg-[#1c2028] text-[#a9abb3] hover:text-white"}`}>
                      {TYPE_LABELS[t]}
                    </button>
                  ))}
                </div>
              ...
              {(type === "BUTTON" || type === "LIST" || type === "MEDIA") && (
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-[#73757d] uppercase tracking-widest">Header <span className="normal-case font-normal">(optional)</span></label>
                  <input value={header} onChange={(e) => setHeader(e.target.value)} placeholder="Header text"
                    className="w-full bg-[#1c2028] border border-[#262B33]/40 rounded-lg px-3 py-2.5 text-sm text-white placeholder:text-[#73757d] focus:outline-none focus:border-[#6366F1]/60" />
                </div>
              )}
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-[#73757d] uppercase tracking-widest">Body</label>
              ...
              {(type === "BUTTON" || type === "LIST") && (
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-[#73757d] uppercase tracking-widest">Footer <span className="normal-case font-normal">(optional)</span></label>
              ...
              {type === "BUTTON" && (
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <label className="text-xs font-bold text-[#73757d] uppercase tracking-widest">Buttons (max 3)</label>
                    {buttons.length < 3 && (
                      <button onClick={() => setButtons((b) => [...b, emptyButton()])} className="text-xs text-[#6366F1] hover:underline">+ Add button</button>
                    )}
                  </div>
                  {buttons.map((btn, i) => (
                    <div key={i} className="bg-[#1c2028] border border-[#262B33]/30 rounded-xl p-4 space-y-3">
                      <div className="flex gap-2">
                        <select value={btn.type} onChange={(e) => updateButton(i, { type: e.target.value as TemplateButton["type"] })}
                          className="bg-[#262B33] text-xs text-white rounded-lg px-2 py-1.5 border border-[#262B33]/40 focus:outline-none">
                          <option value="QUICK_REPLY">Quick Reply</option>
                          <option value="URL">URL</option>
                          <option value="PHONE">Phone</option>
                        </select>
                        <input value={btn.text} onChange={(e) => updateButton(i, { text: e.target.value })} placeholder="Button label"
                          className="flex-1 bg-[#262B33] border border-[#262B33]/40 rounded-lg px-3 py-1.5 text-sm text-white placeholder:text-[#73757d] focus:outline-none" />
                      ...
                      {(btn.type === "URL" || btn.type === "PHONE") && (
                        <input value={btn.value ?? ""} onChange={(e) => updateButton(i, { value: e.target.value })}
                          ...
                          className="w-full bg-[#262B33] border border-[#262B33]/40 rounded-lg px-3 py-1.5 text-sm text-white placeholder:text-[#73757d] focus:outline-none" />
                      )}
                    </div>
                  ))}
                </div>
              ...
              {type === "LIST" && (
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <label className="text-xs font-bold text-[#73757d] uppercase tracking-widest">Sections</label>
                    <button onClick={() => setSections((s) => [...s, emptySection()])} className="text-xs text-[#6366F1] hover:underline">+ Add section</button>
                  </div>
                  {sections.map((sec, si) => (
                    <div key={si} className="bg-[#1c2028] border border-[#262B33]/30 rounded-xl p-4 space-y-3">
                      <input value={sec.title} onChange={(e) => updateSection(si, { title: e.target.value })} placeholder="Section title"
                        className="w-full bg-[#262B33] border border-[#262B33]/40 rounded-lg px-3 py-1.5 text-sm font-semibold text-white placeholder:text-[#73757d] focus:outline-none" />
                      <div className="space-y-2">
                        {sec.rows.map((row, ri) => (
                          <div key={row.id} className="flex gap-2 items-start">
                            <div className="flex-1 space-y-1">
                              <input value={row.title} onChange={(e) => updateRow(si, ri, { title: e.target.value })} placeholder="Item title"
                                className="w-full bg-[#0B0E14] border border-[#262B33]/40 rounded-lg px-3 py-1.5 text-xs text-white placeholder:text-[#73757d] focus:outline-none" />
                              <input value={row.description ?? ""} onChange={(e) => updateRow(si, ri, { description: e.target.value })} placeholder="Description (optional)"
                                className="w-full bg-[#0B0E14] border border-[#262B33]/40 rounded-lg px-3 py-1.5 text-xs text-[#a9abb3] placeholder:text-[#73757d] focus:outline-none" />
                            </div>
                            {sec.rows.length > 1 && (
            ...
            <div className="px-6 pb-6 flex justify-end gap-3">
              <button onClick={() => { setOpen(false); resetForm(); }}
                className="px-4 py-2 rounded-lg text-sm text-[#a9abb3] hover:text-white bg-[#1c2028] transition-colors">Cancel</button>
      ...
      {previewTemplate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setPreviewId(null)}>
          <div className="bg-[#161a21] border border-[#262B33]/40 rounded-2xl w-full max-w-sm p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-bold text-white">{previewTemplate.name}</h3>
              <button onClick={() => setPreviewId(null)} className="text-[#a9abb3] hover:text-white">
                <span className="material-symbols-outlined text-sm">close</span>
              </button>
            </div>
            <div className="bg-[#1c2028] rounded-xl rounded-tl-none p-4 space-y-2 text-sm">
              {previewTemplate.header && <p className="font-bold text-white text-xs">{previewTemplate.header}</p>}
              <p className="text-[#ecedf6] whitespace-pre-wrap">{previewTemplate.content}</p>
              {previewTemplate.footer && <p className="text-[10px] text-[#73757d] italic">{previewTemplate.footer}</p>}
              {previewTemplate.buttons && previewTemplate.buttons.length > 0 && (
                <div className="pt-2 border-t border-[#262B33]/30 space-y-1.5">
              ...
              {previewTemplate.sections && previewTemplate.sections.length > 0 && (
                <div className="pt-2 border-t border-[#262B33]/30">
