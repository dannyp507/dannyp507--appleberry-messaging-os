...
async function upsertPlans() {
  ...
  // WhatsApp Starter — R399/month, 1 account
  await prisma.plan.upsert({
    where: { slug: 'whatsapp-starter' },
    update: {
  ...
  // WhatsApp Growth — R699/month, 2 accounts
  await prisma.plan.upsert({
    where: { slug: 'whatsapp-growth' },
    update: {
  ...
  // WhatsApp Unlimited — R1,200/month, unlimited accounts
  await prisma.plan.upsert({
    where: { slug: 'whatsapp-unlimited' },
    update: {
  ...
  // Beta — owner's unlimited plan, not publicly listed
  await prisma.plan.upsert({
    where: { slug: 'beta' },
    update: {
      name: 'Beta',
