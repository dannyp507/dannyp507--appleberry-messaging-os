...
export class FacebookInboundService {
  ...
  private async processMessage(
    ...
    for (const rule of rules) {
      ...
      await this.maybeTyping(dmSettings, page.pageAccessToken, senderId, rule.useAi ? 0 : 1200);
      ...
      if (rule.useAi) {
        ...
        const aiReply = await this.ai.generateReply(
          { workspaceId, contactId: contact.id, threadId: thread.id, recentMessages, facebookPageId: page.id },
          text,
          rule.response?.trim() || undefined,
        );
        const replyText = aiReply ?? "I'm sorry, I couldn't process that right now.";
        await this.fbPages.sendMessage(page.pageAccessToken, senderId, replyText);
        await this.prisma.inboxMessage.create({
